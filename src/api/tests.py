...
def test_note_create(api_client, user):
    ...
    response = api_client.post(reverse("notes-list"), data={"title": "test", "text": "test"})
    assert response.status_code == status.HTTP_201_CREATED, response.content


def test_note_update(api_client, note):
    api_client.force_login(note.user)
    response = api_client.put(reverse("notes-detail", args=(note.id,)), data={"title": "new_title", "text": "test"})
